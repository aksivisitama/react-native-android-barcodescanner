...
class RCTCameraViewFinder extends TextureView implements TextureView.SurfaceTextureListener, Camera.PreviewCallback {
    ...
    private Runnable doAutoFocus = new Runnable() {
        public void run() {
            RCTCameraViewFinder.this.safeAutoFocus();
        }
    };
    Camera.AutoFocusCallback autoFocusCB = new Camera.AutoFocusCallback() {
        public void onAutoFocus(boolean success, Camera camera) {
            RCTCameraViewFinder.this.scheduleAutoFocus();
        }
    };
    ...
    private void setupScanner() {
        ...
        List<BarcodeFormat> barcodeFormat = new ArrayList<>();
        barcodeFormat.add(BarcodeFormat.QRCODE);
        barcodeFormat.add(BarcodeFormat.CODE128);

        Iterator i$ = barcodeFormat.iterator();
        ...
        try {
            if (_camera != null) {
                ...
                if (parameters.getFocusMode().equals(Camera.Parameters.FOCUS_MODE_AUTO)) {
                    _camera.cancelAutoFocus();
                    _camera.autoFocus(this.autoFocusCB);
                }
            }
        } catch (Exception e) {
            e.printStackTrace();
